...
class Transactions {
	...
	public static function getallcurriences($count=false,$page=false,$per_page=false,$c_currency=false,$currency=false,$user=false,$start_date=false,$type=false,$order_by=false,$order_desc=false,$public_api_all=false,$dont_paginate=false) {
		global $CFG;
		if ($user && !(User::$info['id'] > 0))
			return false;
		...
		if (!$count && !$public_api_all)
			$sql = "SELECT transactions.id,transactions.c_currency, transactions.date,transactions.site_user,transactions.site_user1,transactions.btc,transactions.currency,transactions.currency1,transactions.btc_price,transactions.orig_btc_price,transactions.fiat, (UNIX_TIMESTAMP(transactions.date) - ({$CFG->timezone_offset})) AS time_since,transaction_types.name_{$CFG->language} AS type, transactions.fee AS fee, transactions.btc_net AS btc_net, transactions.orig_btc_price AS fiat_price";
		// elseif ($public_api_all && $user)
	...
	public static function getnew($count=false,$page=false,$per_page=false,$c_currency=false,$currency=false,$user=false,$start_date=false,$type=false,$order_by=false,$order_desc=false,$public_api_all=false,$dont_paginate=false) {
		global $CFG;
		// return "hello4";
		// if ($user && !(User::$info['id'] > 0))
		// 	return false;
		...
		$sql .= ' and transactions.c_currency = "'.$c_currency.'" and transactions.currency = "'.$currency.'"';
	...
	public static function get24hData($c_currency=false,$currency=false) {
		...
		$result = db_query_array($sql);
		$lastPrice = number_format($result[0]['btc_price'],'8','.','') ;
		
		$transactions_24h_data['lastPrice'] = $lastPrice ;
		$transactions_24h_data['change_24hrs'] = number_format($change_24hrs,'8','.','') ;
	...
	public static function get24LowestData($c_currency=false,$currency=false) {
		global $CFG;
		
		$sql = "SELECT MIN(`fiat_after`) AS  `lowest` FROM transactions WHERE date >= DATE(NOW() - INTERVAL 1 DAY) AND currency = ".$currency." And c_currency = ".$c_currency."";
		$result = db_query_array($sql);
		
		return $result;
	}
	public static function getLowestData($c_currency=false,$currency=false) {
		global $CFG;
		
		$sql = "SELECT MIN(`fiat_after`) AS  `low` FROM transactions WHERE date == date() AND currency = ".$currency." And c_currency = ".$c_currency."";
		$result = db_query_array($sql);
		
		return $result;
	}
	public static function gethighData($c_currency=false,$currency=false) {
		global $CFG;
		
		$sql = "SELECT MAX(`fiat_after`) AS  `high` FROM transactions WHERE date == date() AND currency = ".$currency." And c_currency = ".$c_currency."";
		$result = db_query_array($sql);
