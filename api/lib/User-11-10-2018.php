...
class User {
	...
	public static function getRegistereKYCReferal($user_id) {


		$sql = 'SELECT user_id FROM sessions WHERE session_id = '.$user_id;
			$result = db_query_array($sql);
			// return $result[0];

		$sql1 = 'SELECT email FROM site_users WHERE id = '.$result[0]['user_id'];
			$result1 = db_query_array($sql1);
			return $result1[0];
