...
?>
<table class="table table-striped order-table-fixed right-data-table trade_history_table1">
<thead>
<tr>
...
<tbody>
<?
if ($transactions_market) {
foreach ($transactions_market as $transaction_market) {
$trans_symbol = $CFG->currencies[$transaction_market['currency']]['fa_symbol'];
echo '
<tr id="transaction_' . $transaction_market['id'] . '">
<input type="hidden" class="is_crypto" value="' . $transaction_market['is_crypto'] . '" />
<td>' . $transaction_market['type'] . '</td>
<td>'.$transaction_market['date'].'<input type="hidden" class="localdate" value="' . (strtotime($transaction_market['date'])) . '" /></td>
<td>' . Stringz::currency($transaction_market['btc'], true) . ' ' . $CFG->currencies[$transaction_market['c_currency']]['fa_symbol'] . '</td>
<td><span class="currency_char">' . $trans_symbol . '</span><span>' . Stringz::currency($transaction_market['btc_net'] * $transaction_market['fiat_price'], ($transaction_market['is_crypto'] == 'Y')) . '</span></td>
<td><span class="currency_char">' . $trans_symbol . '</span><span>' . Stringz::currency($transaction_market['fiat_price'], ($transaction_market['is_crypto'] == 'Y')) . '</span></td>
<td><span class="currency_char">' . $trans_symbol . '</span><span>' . number_format(Stringz::currency($transaction_market['fee'] * $transaction_market['fiat_price'], ($transaction_market['is_crypto'] == 'Y')),'8','.','') . '</span></td>
</tr>';
}
}else{
echo '<tr id="no_transactions" style="' . (is_array($transactions_market) ? 'display:none;' : '') . '"><td colspan="6" style="padding: 0;"><div class="" style="background: #f4f6f8; text-align:  center;
...
$transactions_my_trade = $query['Transactions']['get']['results'][0];
$transaction_types = $query['Transactions']['getTypes']['results'][0];

if ($transactions_my_trade) {
foreach ($transactions_my_trade as $transaction_mt) {
$trans_symbol = $CFG->currencies[$transaction_mt['currency']]['fa_symbol'];
echo '
<tr id="transaction_' . $transaction_mt['id'] . '">
<input type="hidden" class="is_crypto" value="' . $transaction_mt['is_crypto'] . '" />
<td>' . $transaction_mt['type'] . '</td>
<td><input type="hidden" class="localdate" value="' . (strtotime($transaction_mt['date'])) . '" /></td>
<td>' . Stringz::currency($transaction_mt['btc'], true) . ' ' . $CFG->currencies[$transaction_mt['c_currency']]['fa_symbol'] . '</td>
<td><span class="currency_char">' . $trans_symbol . '</span><span>' . Stringz::currency($transaction_mt['btc_net'] * $transaction_mt['fiat_price'], ($transaction_mt['is_crypto'] == 'Y')) . '</span></td>
<td><span class="currency_char">' . $trans_symbol . '</span><span>' . Stringz::currency($transaction_mt['fiat_price'], ($transaction_mt['is_crypto'] == 'Y')) . '</span></td>
<td><span class="currency_char">' . $trans_symbol . '</span><span>' . Stringz::currency($transaction_mt['fee'] * $transaction_mt['fiat_price'], ($transaction_mt['is_crypto'] == 'Y')) . '</span></td>
</tr>';
}
}else{
