...
if ($endpoint == 'market-price') {		
		...
		API::add('User','getRow',array('currencies','id','where currency="'.$market_input[0].'"'));
	    $query = API::send();
	    $para1 = $query['User']['getRow']['results'][0]['id']; 
	    API::add('User','getRow',array('currencies','id','where currency="'.$market_input[1].'"'));
	    $query = API::send();
	    $para2 = $query['User']['getRow']['results'][0]['id']; 
	    if($para1!='' && $para2!='')
	    {
		API::add('Transactions','get24hData',array($para1, $para2));
		$query = API::send();		
		$currentPair = $query['Transactions']['get24hData']['results'][0];
		$return['market-price'] = number_format($currentPair['lastPrice'],'8','.','');
		}
		else
		{
		$return['errors'][] = array('message'=>'Invalid Currency Pair','code'=>'INVALID_CURRENCY_PAIR');	
		}
}
