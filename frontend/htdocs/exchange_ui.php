...
ini_set('display_errors', 1);
    ...
    // API::add('Transactions','getLowestData',array($c_currency1,$currency1)); 
    // API::add('Transactions','gethighData',array($c_currency1,$currency1)); 
   ...
   // print_r($query);die(); 
    ...
    // $lowprice = $query['Transactions']['getLowestData']['results'][0][0]['low'];
    // $highprice = $query['Transactions']['gethighData']['results'][0][0]['high'];
    // echo $lowprice;
    // echo $highprice;
    // echo $low24price;
    ...
    if ($buy && !is_array(Errors::$errors)) {
       ...
       if (!$errors1 && !$errors2){
        ...
        Link::redirect('exchange_ui?c_currency='.$_REQUEST["c_currency"].'&currency='.$_REQUEST["currency"].'&transactions='.$operations["transactions"].'new_order=1');
        ...
        Link::redirect('exchange_ui?c_currency='.$_REQUEST["c_currency"].'&currency='.$_REQUEST["currency"].'&transactions='.$operations["transactions"].'');
    ...
    if ($sell && !is_array(Errors::$errors)) {
        ...
        Link::redirect('exchange_ui?c_currency='.$_REQUEST["c_currency"].'&currency='.$_REQUEST["currency"].'&transactions='.$operations["transactions"].''); //newly added 
        exit;
        }
        }
    }
...
<head>
    ...
    <style>
        ...
        ::-webkit-scrollbar-track {
            box-shadow: inset 0 0 5px #b3d5d6 !important; 
            border-radius: 10px;
        }
...
tr.clickable-row:hover td:nth-child(1) label svg.svg-inline--fa.fa-star.fa-w-18 path {
    fill: #d6e6e6 !important;
}
tr.clickable-row:hover td:nth-child(1) {
...
tr.clickable-row.active td:nth-child(1) label svg.svg-inline--fa.fa-star.fa-w-18 path {
    fill: #1e2324 !important;
}
tr.clickable-row.active td:nth-child(1) {
    background-color: #f7ab1c;
}
.tab-box .table td span {
    font-weight: normal;
}
.form-box .form-head p span {
...
input.Flex__Flex-fVJVYW.ghkoKS.buy-btc {
    /*background-color: unset;*/
    box-shadow: unset;
    border: unset;
}
...
/* Trade History Table*/
.trade-history ul#myTab {
    border-bottom: 0px solid #e39706;
}
.trade-history ul#myTab {
    border-bottom: 0px solid #e39706;
}
.trade-history .nav-tabs.btn-tab .nav-item .nav-link {
    color: #3a4449 !important;
}
.trade-history li.nav-item .nav-link {
    border: 1px solid #91abac !important;
    background-color: #ffffff !important;
    background-image: linear-gradient(#ffffff, #c4d5db) !important;
    box-shadow: none;
    font-size: 14px !important;
...
.trade-history li.nav-item .nav-link.active {
    background-color: #e9ad3c !important;
    background-image: -webkit-linear-gradient(#e9ad3c, #fbeed6);
    background-image: linear-gradient(#e9ad3c, #fbeed6) !important;
    color: #342201 !important;
}
.trade-history label.order-title {
    background-color: #ffffff;
    color: #1e2324;
    font-size: 22px;
    position: absolute;
...
.amcharts-export-menu.amcharts-export-menu-bottom-right.amExportButton {
    display: none;
}
    </style>
</head>
...
<body id="wrapper">
        ...
        <div class="container-fluid">
            <div class="row ">
                <div class="col-md-9 col-sm-8 col-xs-12">
                    <div class="row chart-title-head">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <div class="chart-title">
                                <h6 class="name">BitExchange</h6>
 ...
 <input type="hidden" value="<?php echo $newDate;?>" id="hello2">
    ...
    //   }
    // }
        var candle_n_chart_data = [];
        var candle_api_url = "chart_json.php?currency=<?php echo $currency_id1;?>&c_currency=<?php echo $c_currency_id; ?>";
      $.ajax({
                type: "GET",
                url: candle_api_url,
                dataType:'json',
                ...
                success: function(data){
                  ...
                  data.Data.forEach(function(element) {
                    ...
                    var high_value = Number(element.highest)+(10);
                    candle_n_chart_data.push( {
                      "date": element.date,
                      "open": element.highest,
                      "high": high_value,
                      "low": element.lowest-10,
                      "close": element.lowest
                    } );
                    // console.log("single Data :"+JSON.stringify(candle_n_chart_data));
                    $('#chartdiv1').val(JSON.stringify(candle_n_chart_data));
                  });
 ...
 console.log("single Data :"+JSON.stringify(candle_n_chart_data));
                  if (data.Data.length == 0) {
                    ...
                    candle_n_chart_data.push( {
                     ...
                     // candle_n_chart_data=JSON.stringify(candle_n_chart_data);
  ...
  "graphs": [ {
    "id": "g1",
    "balloonText": "Low:<b>[[close]]</b><br>High:<b>[[open]]</b><br>",
    "closeField": "close",
    "fillColors": "#7f8da9",
    "highField": "high",
    "lineColor": "#7f8da9",
  ...
  "dataProvider": candle_n_chart_data,
                    ...
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="row">
                               ...
                               <?php }
                                    ...
                                    <ul class="nav nav-tabs btn-tab" id="myTab" role="tablist">
                                        <li class="nav-item">
                                            <a class="nav-link active" id="mar-trades-tab" data-toggle="tab" href="#my-trades1" role="tab" aria-controls="mar-trades" aria-selected="false">Market Trades</a>
                                        </li>
                                        <?php
                                            if(User::isLoggedIn()){
                                            ?>
                                        <li class="nav-item">
                                            <a class="nav-link" id="my-trades-tab" data-toggle="tab" href="#mar-trades" role="tab" aria-controls="my-trades1" aria-selected="false">My Trades</a>
                                        </li>
                                        <?php
                                            }
                                        ?>
                                ...
                                <div class="tab-pane fade" id="mar-trades" role="tabpanel" aria-labelledby="mar-trades-tab">
                            ...
                            <input type="hidden" id="page" value="<?= $page1 ?>" />
                                        ...
                                        if ($transactions) {
                                            foreach ($transactions as $transaction) {
                                                ...
                                                <tr id="transaction_' . $transaction['id'] . '">
                                                    ...
                                                    <td><span class="currency_char">' . $trans_symbol . '</span><span>' . Stringz::currency11($transaction['fee'] * $transaction['fiat_price'], ($transaction['is_crypto'] == 'Y')) . '</span></td>
                                                </tr>';
                                            }
                                        }else{
                                ...
                                <div class="tab-pane fade show active" id="my-trades1" role="tabpanel" aria-labelledby="my-trades-tab">
                                	<?php
                                        ...
                                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                                            ...
                                            <li class="nav-item">
                                                ...
                                                if($active_currency_id == 27)
                                                    ...
                                                    $class_active = "";
                                            ?>
                                                <a class="nav-link <?php echo $class_active;?>" id="r-usd-tab" data-toggle="tab" href="#r-usd" role="tab" aria-controls="r-usd" aria-selected="false">USD</a>
                                            </li>
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="news-box">
                                ...
                                <div class="news-inner">
                                    <div class="content">
                                        <a class="twitter-timeline" href="https://twitter.com/ExchangeScript?ref_src=twsrc%5Etfw">Tweets by ExchangeScript</a> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
                                    </div>
                                </div>
                            </div>
                        </div>
...
jQuery(document).ready(function($) {
 ...
 <script>
        ...
        $('.buy-active').on('click', function(){
             ...
             $('#stop-buy').addClass("active");
              $('#stop-sell').removeClass("active");
              $('#stop-sell').css("display","none");
              $('#stop-buy').css("display","block");
        })
        $('.sell-active').on('click', function(){
             $('.buy-active').removeClass("active");
              $('.sell-active').addClass("active");
             $('#stop-buy').removeClass("active");
              $('#stop-sell').addClass("active");
              $('#stop-sell').css("display","block");
              $('#stop-buy').css("display","none");
        })
		...
		function order_stop_buy_confirm()
		{
			...
			$.ajax ({
			   ...
			   success: function(respon)
			   {
					...
					// trade_history_table
					$('.trade_history_block').html(respon[10]);
					...
					setTimeout(function(){ $('.success_message').hide('slow'); $('.errors').hide('slow'); }, 5000);
			   }
		   })
			   ...
			   success: function(respon)
			   {
					...
					setTimeout(function(){
						...
						{                            
							...
							// Market Block
							$('#market_block').html(respon[4]);
							// End of Market Block

							// Balance Block
							$('.buy_user_available').html(respon[5]);
							$('.sell_user_available').html(respon[6]);
							// End of Balance Block

							// buy_orders_table
							$('#buy_orders_table').html(respon[7]);
							// End of buy_orders_table

							// sell_orders_table
							$('#sell_orders_table').html(respon[8]);
							// End of sell_orders_table                            

							// trade_history_table
							$('.trade_history_block').html(respon[9]);
							// End of trade_history_table
						}
						else
						{
