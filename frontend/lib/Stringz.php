...
class Stringz {
	...
	public static function currency($amount,$crypto=false,$flex=false) {
		...
		if (!empty($amount) && $amount > 0) {
			...
			$dec_amount = (!is_numeric($crypto)) ? ($crypto ? 8 : 8) : $crypto;
			...
			return number_format($amount,$dec_amount,$decimal,$thousands); //$amount;
		}
		else {
			return 0;
		}
	...
	public static function currency1($amount,$crypto=false,$flex=false) {
		global $CFG;
		if (!empty($amount) && $amount > 0) {
			$thousands = ($CFG->thousands_separator) ? $CFG->thousands_separator : ',';
			$decimal = ($CFG->decimal_separator) ? $CFG->decimal_separator : '.';
	...
	public static function currency11($amount,$crypto=false,$flex=false) {
		global $CFG;
		if (!empty($amount) && $amount > 0) {
			$thousands = ($CFG->thousands_separator) ? $CFG->thousands_separator : ',';
			$decimal = ($CFG->decimal_separator) ? $CFG->decimal_separator : '.';
			$dec_amount = (!is_numeric($crypto)) ? ($crypto ? 8 : 10) : $crypto;
